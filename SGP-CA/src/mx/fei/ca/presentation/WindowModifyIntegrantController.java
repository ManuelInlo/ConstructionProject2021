...
/**
 * Clase para representar el controlador del FXML WindowModifyIntegrant
 * @author Gloria Mendoza González
 * @version 17-06-2021
 */

public class WindowModifyIntegrantController implements Initializable {
    ...
    /**
     * Método que manda a modificar al integrante con la información obtenida de los campos de la GUI
    ...
    @FXML
    private void closeModifyIntegrant(ActionEvent event) {
   ...
   /**
    ...
    /**
     * Método que llena los campos de la GUI con la información del integrante a modificar
     * @param integrant Define el integrante a modificar
     * @throws mx.fei.ca.businesslogic.exceptions.BusinessConnectionException
     */
    
    public void fillFieldsIntegrants(Integrant integrant) throws BusinessConnectionException{
        tfCurp.setText(integrant.getCurp());
        cbRole.getSelectionModel().select(integrant.getRole());
        tfNameIntegrant.setText(integrant.getNameIntegrant());
        cbStudyDegree.getSelectionModel().select(integrant.getStudyDegree());
        tfStudyDiscipline.setText(integrant.getStudyDiscipline());  
        cbProdepParticipation.getSelectionModel().select(integrant.getProdepParticipation());
        tfTypeTeaching.setText(integrant.getTypeTeaching());
        tfIesStudyDegree.setText(integrant.getStudyDegree());
        tfInstitutionalMail.setText(integrant.getInstitutionalMail());
        tfNumberPhone.setText(integrant.getNumberPhone());
        dpDateBirthday.setValue(integrant.getDateBirthday().toLocalDate());
        cbStatusIntegrant.getSelectionModel().select(integrant.getStatusIntegrant());   
        LgacDAO lgacDAO = new LgacDAO();  
        if(lgacDAO.findFirstLgacOfIntegrant(integrant.getCurp())){
            checkBoxL1.setSelected(true);
        }        
        if(lgacDAO.findSecondLgacOfIntegrant(integrant.getCurp())){
            checkBoxL2.setSelected(true);
        }        
    }   
    ...
    private boolean existsDuplicateValues() throws BusinessConnectionException{
        ...
        if(integrantDAO.existsIntegrantNameForUpdate(tfNameIntegrant.getText(), integrant.getCurp())){  
            duplicateName = true;
            TypeError typeError = TypeError.NAMEDUPLICATE;
            showInvalidFieldAlert(typeError);
        }
        
        if(integrantDAO.existsIntegrantEmailForUpdate(tfInstitutionalMail.getText(), integrant.getCurp())){
            duplicateEmail = true;
